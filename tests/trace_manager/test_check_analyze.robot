...
test_1
    ...
    IF    $ver != '2.6'    Check Check Box    checkBoxexplain_plan
    ...
    ${row}=    Find Table Row
    ...    0
    ...    SELECT * from EMPLOYEE AS EM JOIN DEPARTMENT AS DEP ON EM.DEPT_NO = DEP.DEPT_NO\n
    ...    QUERY
    ...
    ELSE IF    $ver == '3.0'
        VAR    ${expected_plan}=
        ...    Select Expression -> Nested Loop Join (inner) -> Table "DEPARTMENT" as "DEP" Full Scan -> Filter -> Table "EMPLOYEE" as "EM" Access By ID -> Bitmap -> Index "RDB$FOREIGN8" Range Scan (full match)
    ELSE IF    $ver == '5.0'
        VAR    ${expected_plan}=
        ...    Select Expression -> Filter -> Hash Join (inner) -> Table "EMPLOYEE" as "EM" Full Scan -> Record Buffer (record length: 105) -> Table "DEPARTMENT" as "DEP" Full Scan
    END
