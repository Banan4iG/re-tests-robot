...
*** Keywords ***
Init Edit Column
    [Arguments]    ${query}=CREATE TABLE TEST_TABLE (TEST_COLUMN INT)
    Lock Employee
    Execute Immediate    ${query}
    Open connection
    Click On Tree Node    0    New Connection|Tables (11)|TEST_TABLE    2
    Select Tab    Columns
    ${row}=    Find Table Row    0    TEST_COLUMN    Name
...
Check SQL Statements
    ...
    Should Be Equal As Strings    ${alter_table}    ALTER TABLE TEST_TABLE ALTER COLUMN TEST_COLUMN DROP IDENTITY    strip_spaces=${True}    collapse_spaces=${True}
    ...
    IF    ${check_sequence}
        ...
        Should Be Equal As Strings    ${create_sequence}    CREATE OR ALTER SEQUENCE AUTO_GEN START WITH 1 INCREMENT BY 1    strip_spaces=${True}    collapse_spaces=${True}
    END
