...
test_remove_not_null
    Check Skip 26
    Init Edit Column    RECREATE TABLE TEST_TABLE (TEST_COLUMN INT NOT NULL)
...
test_remove_default
    Init Edit Column    RECREATE TABLE TEST_TABLE (TEST_COLUMN INT DEFAULT 100)
    Select Tab As Context    Default Value
    Clear Text Field    0
    Select Main Window
    Select Dialog    Edit table column
...
test_computed_by
    Init Edit Column    RECREATE TABLE TEST_TABLE (TEST_COLUMN_2 INT, TEST_COLUMN INT COMPUTED BY (TEST_COLUMN_2 * 2))
...
test_identity_to_always
    Check Skip
    Init Edit Column    RECREATE TABLE TEST_TABLE (TEST_COLUMN INT GENERATED BY DEFAULT AS IDENTITY)
    ...
    ...    STARTS:DatabaseError:
    ...    Execute Immediate
    ...    INSERT INTO TEST_TABLE (TEST_COLUMN) VALUES (10)
    ${res}=    Execute    SELECT * FROM TEST_TABLE
    Should Be Equal As Strings    ${res}    [(1,), (2,)]

test_identity_to_by_default
    Check Skip
    Init Edit Column    RECREATE TABLE TEST_TABLE (TEST_COLUMN INT GENERATED ALWAYS AS IDENTITY)
    Check Box Should Be Checked    incrementedCheck
...
test_add_new_gen_without_identity
    Init Edit Column    query=RECREATE TABLE TEST_TABLE (COL INT)    column_name=COL
...
test_check_autogen_seq_name
    Init Edit Column    RECREATE TABLE TEST_TABLE (NEW_TABLE_COLUMN_1 INT, NEW_TABLE_COLUMN_2 INT)    NEW_TABLE_COLUMN_2
    Check Check Box    incrementedCheck
    Select Tab    Autoincrement
    Push Radio Button    newSequenceRadio
...
test_type_to_domain
    Init Edit Column    RECREATE TABLE TEST_TABLE (TEST_COLUMN VARCHAR(30))
...
test_domain_to_type
    Init Edit Column    RECREATE TABLE TEST_TABLE (TEST_COLUMN ADDRESSLINE)
...
test_check_ddl_to_create
    Init Edit Column    RECREATE TABLE TEST_TABLE (TEST_COLUMN VARCHAR(30) DEFAULT 'test' NOT NULL)
    Select Tab As Context    DDL to create
    ${ddl}=    Get Text Field Value    0
    Should Be Equal As Strings
    ...    ${ddl}
    ...    ALTER TABLE TEST_TABLE ADD TEST_COLUMN VARCHAR(30) DEFAULT 'test' NOT NULL;
    ...    strip_spaces=${True}
...
*** Keywords ***
Init Edit Column
    [Arguments]    ${query}=RECREATE TABLE TEST_TABLE (TEST_COLUMN INT)    ${column_name}=TEST_COLUMN
    Lock Employee
...
Check SQL Statements
    ...
    IF    ${check_sequence}
        ...
        ELSE
            ...
            Should Be Equal As Strings
            ...    ${alter_sequence}
            ...    ALTER SEQUENCE ${gen_name} RESTART WITH 1
            ...    strip_spaces=${True}
            ...    collapse_spaces=${True}
        END
    END
