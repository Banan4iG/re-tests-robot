...
alter_ddl
    ...
    ...    ${ddl}
    ...    CREATE OR ALTER PROCEDURE TEST${authid}RETURNS ( TOT INTEGER ) AS BEGIN tot = 66; END;
    ...    strip_spaces=${True}
...
alter_add_cursor
    ...
    IF    ${{$ver != '2.6'}}    Click On Table Cell    0    1    Scroll
    ...
    IF    ${{$ver != '2.6'}}
        VAR    ${expected_ddl}=
        ...    CREATE OR ALTER PROCEDURE TEST RETURNS ( PAR0 INTEGER ) AS DECLARE PAR1 CURSOR FOR (SELECT * FROM EMPLOYEE); DECLARE PAR2 SCROLL CURSOR FOR (SELECT * FROM COUNTRY); BEGIN PAR0 = 55; END;
    ELSE
        VAR    ${expected_ddl}=
        ...    CREATE OR ALTER PROCEDURE TEST AUTHID OWNER RETURNS ( PAR0 INTEGER ) AS DECLARE PAR1 CURSOR FOR (SELECT * FROM EMPLOYEE); DECLARE PAR2 CURSOR FOR (SELECT * FROM COUNTRY); BEGIN PAR0 = 55; END;
    END
