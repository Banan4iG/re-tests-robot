...
test_CSV_export_to_folder
    ...
    IF    ${{$ver != '2.6'}}
        ...
        ...    SEPARATOR=\n
        ...    VBASE;Video Database;${blob_paths}[0];45;software
        ...    DGPII;DigiPizza;${blob_paths}[1];24;other
        ...    GUIDE;AutoMap;${blob_paths}[2];20;hardware
        ...    MAPDB;MapBrowser port;${blob_paths}[3];4;software
        ...    HWRII;Translator upgrade;${blob_paths}[4];;software
        ...    MKTPR;Marketing project 3;${blob_paths}[5];85;N/A
        ...    FCORE;Firebird engine;${blob_paths}[6];146;software
        ...    FBDOC;Documentation;${blob_paths}[7];;other
        ...    PYTHN;Python drivers;${blob_paths}[8];151;software
        ...    DTNET;.NET drivers;${blob_paths}[9];152;software
        ...    ODBCD;ODBC drivers;${blob_paths}[10];;software
        ...    PHPDR;PHP drivers;${blob_paths}[11];;software
        ...    JAVAD;Java drivers;${blob_paths}[12];153;software
        ...    FB-QA;Firebrid QA;${blob_paths}[13];150;software
        ...    INFRA;Infrastructure;${blob_paths}[14];;other
        ...    BTLER;Firebrid Butler;${blob_paths}[15];151;software
        ...    ${EMPTY}
    ELSE
        ${expected_content}=    Catenate
        ...    SEPARATOR=\n
...
test_CSV_export_to_file
    ...
    IF    ${{$ver != '2.6'}}
        ${expected_content}=    Catenate
        ...    SEPARATOR=\n
        ...    VBASE;Video Database;:h00000000_00000059;45;software
        ...    DGPII;DigiPizza;:h00000059_00000077;24;other
        ...    GUIDE;AutoMap;:h000000d0_00000055;20;hardware
        ...    MAPDB;MapBrowser port;:h00000125_00000048;4;software
        ...    HWRII;Translator upgrade;:h0000016d_00000056;;software
        ...    MKTPR;Marketing project 3;:h000001c3_00000061;85;N/A
        ...    FCORE;Firebird engine;:h00000224_00000087;146;software
        ...    FBDOC;Documentation;:h000002ab_00000046;;other
        ...    PYTHN;Python drivers;:h000002f1_00000028;151;software
        ...    DTNET;.NET drivers;:h00000319_00000026;152;software
        ...    ODBCD;ODBC drivers;:h0000033f_00000015;;software
        ...    PHPDR;PHP drivers;:h00000354_00000025;;software
        ...    JAVAD;Java drivers;:h00000379_00000026;153;software
        ...    FB-QA;Firebrid QA;:h0000039f_0000001d;150;software
        ...    INFRA;Infrastructure;:h000003bc_00000021;;other
        ...    BTLER;Firebrid Butler;:h000003dd_0000002d;151;software
        ...    ${EMPTY}
    ELSE
        ${expected_content}=    Catenate
        ...    SEPARATOR=\n
...
Check Blobs In Folder
    ...
    Should Be Equal As Strings
    ...    ${contents}[3]
    ...    Port the map browsing database software to run on the automobile model.
    ...    strip_spaces=${True}
    ...    collapse_spaces=${True}
    Should Be Equal As Strings
    ...    ${contents}[4]
    ...    Integrate the hand-writing recognition module into the universal language translator.
    ...    strip_spaces=${True}
    ...    collapse_spaces=${True}
    Should Be Equal As Strings
    ...    ${contents}[5]
    ...    Expand marketing and sales in the Pacific Rim. Set up a field office in Australia and Singapore.
    ...    strip_spaces=${True}
    ...    collapse_spaces=${True}
    ...
    IF    ${{$ver != '2.6'}}
        Should Be Equal As Strings
        ...    ${contents}[6]
        ...    Everything that makes core Firebird distributions: - Firebird server - Utilities - Plugins, UDF, UDR - Ports, packaging and installers
        ...    strip_spaces=${True}
        ...    collapse_spaces=${True}
        Should Be Equal As Strings
        ...    ${contents}[7]
        ...    Firebird documentation: - Release Notes - Guides and books - Articles
        ...    strip_spaces=${True}
        ...    collapse_spaces=${True}
        Should Be Equal As Strings
        ...    ${contents}[8]
        ...
        Should Be Equal As Strings
        ...    ${contents}[15]
        ...    Firebird Butler: - Specifications - Saturnin
        ...    strip_spaces=${True}
        ...    collapse_spaces=${True}
    END
