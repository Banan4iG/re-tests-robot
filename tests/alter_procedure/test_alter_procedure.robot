...
alter_ddl
    ...
    Should Be Equal As Strings    ${ddl}    CREATE OR ALTER PROCEDURE TEST RETURNS ( TOT INTEGER ) AS BEGIN tot = 66; END    strip_spaces=${True}    collapse_spaces=${True}
...
alter_remove_out_par
    Remove par    CREATE OR ALTER PROCEDURE TEST RETURNS ( PAR0 INTEGER, PAR1 VARCHAR(10) CHARACTER SET UTF8 NOT NULL, PAR2 EMPNO, PAR3 TYPE OF CUSTNO, PAR4 TYPE OF COLUMN COUNTRY.COUNTRY ) AS BEGIN PAR0 = 55; END    Output Parameters    CREATE OR ALTER PROCEDURE TEST RETURNS ( PAR0 INTEGER ) AS BEGIN PAR0 = 55; END    
...
move_out_par
    Move par    CREATE OR ALTER PROCEDURE TEST RETURNS ( PAR1 INTEGER, PAR2 INTEGER, PAR3 INTEGER, PAR4 INTEGER ) AS BEGIN END    Output Parameters    CREATE OR ALTER PROCEDURE TEST RETURNS ( PAR1 INTEGER, PAR3 INTEGER, PAR2 INTEGER, PAR4 INTEGER ) AS BEGIN END

move_cursor
    Move par    CREATE OR ALTER PROCEDURE TEST RETURNS ( TOT INTEGER ) AS DECLARE PAR0 INTEGER; DECLARE PAR1 CURSOR FOR (SELECT * FROM EMPLOYEE); DECLARE PAR2 CURSOR FOR (SELECT * FROM EMPLOYEE); DECLARE PAR3 CURSOR FOR (SELECT * FROM EMPLOYEE); DECLARE PAR4 CURSOR FOR (SELECT * FROM EMPLOYEE); BEGIN tot = 55; END    Cursors    CREATE OR ALTER PROCEDURE TEST RETURNS ( TOT INTEGER ) AS DECLARE PAR0 INTEGER; DECLARE PAR1 CURSOR FOR (SELECT * FROM EMPLOYEE); DECLARE PAR2 CURSOR FOR (SELECT * FROM EMPLOYEE); DECLARE PAR3 CURSOR FOR (SELECT * FROM EMPLOYEE); DECLARE PAR4 CURSOR FOR (SELECT * FROM EMPLOYEE); BEGIN tot = 55; END

move_var 
    Move par    CREATE OR ALTER PROCEDURE TEST RETURNS ( TOT INTEGER ) AS DECLARE PAR0 INTEGER; DECLARE PAR1 VARCHAR(10) CHARACTER SET UTF8 NOT NULL; DECLARE PAR2 EMPNO; DECLARE PAR3 TYPE OF CUSTNO; DECLARE PAR4 TYPE OF COLUMN COUNTRY.COUNTRY; BEGIN tot = 55; END    Variables    CREATE OR ALTER PROCEDURE TEST RETURNS ( TOT INTEGER ) AS DECLARE PAR0 INTEGER; DECLARE PAR1 VARCHAR(10) CHARACTER SET UTF8 NOT NULL; DECLARE PAR2 EMPNO; DECLARE PAR3 TYPE OF CUSTNO; DECLARE PAR4 TYPE OF COLUMN COUNTRY.COUNTRY; BEGIN tot = 55; END
