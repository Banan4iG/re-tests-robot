...
test_external_module
    ...
    Check Changes
    ...    CREATE OR ALTER TRIGGER TEST_TRIGGER FOR COUNTRY ACTIVE BEFORE INSERT POSITION 0 EXTERNAL NAME 'TestExternalMetod' ENGINE TestEngine
...
Init No Dependencies
    ...
    Lock Employee
    Execute Immediate
    ...    CREATE OR ALTER TRIGGER TEST_TRIGGER FOR COUNTRY ACTIVE BEFORE INSERT POSITION 0 AS BEGIN /* Comment */ END
    Open Connection
    Select From Tree Node Popup Menu    0    New Connection|Table Triggers (5)|${name}    Edit table trigger
