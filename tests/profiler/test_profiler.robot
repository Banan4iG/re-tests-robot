...
test_compact_view
    ...
    ...    ${values}[:-1]
    ...    ['Profiler Session [ID: 1]', 'execute procedure SUB_TOT_BUDGET(600)']
    ...
    Should Be Equal As Strings
    ...    ${values}[:-1]
    ...    ['Profiler Session [ID: 1]', 'execute procedure SUB_TOT_BUDGET(600)', 'SUB_TOT_BUDGET', 'Self Time']
    ...
    Should Be Equal As Strings
    ...    ${values}[:-1]
    ...    ['Profiler Session [ID: 1]', 'execute procedure SUB_TOT_BUDGET(600)', 'SUB_TOT_BUDGET', '2: SELECT SUM(budget), AVG(budget), MIN(budget), MAX(budget)']
...
test_round
    ...
    ${row}=    Find Table Row
    ...    0
    ...    EXECUTE BLOCK AS DECLARE I INTEGER; BEGIN I = 0; WHILE ( I <> 100000) DO BEGIN I = I + 1; END end
    Should Not Be Equal As Integers    ${row}    -1
...
test_auto-reload_tree
    ...
    Should Be Equal As Strings
    ...    ${values}
    ...    ['COUNTRY', 'CUSTOMER', 'DEPARTMENT', 'EMPLOYEE', 'EMPLOYEE_PROJECT', 'JOB', 'PLG$PROF_CURSORS', 'PLG$PROF_PSQL_STATS', 'PLG$PROF_RECORD_SOURCES', 'PLG$PROF_RECORD_SOURCE_STATS', 'PLG$PROF_REQUESTS', 'PLG$PROF_SESSIONS', 'PLG$PROF_STATEMENTS', 'PROJECT', 'PROJ_DEPT_BUDGET', 'SALARY_HISTORY', 'SALES']

    @{values}=    Get Tree Node Child Names    0    New Connection|Views (4)
    Should Be Equal As Strings
    ...    ${values}
    ...    ['PHONE_LIST', 'PLG$PROF_PSQL_STATS_VIEW', 'PLG$PROF_RECORD_SOURCE_STATS_VIEW', 'PLG$PROF_STATEMENT_STATS_VIEW']
